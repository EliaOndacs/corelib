...
from typing import Any, Generator, Iterable, Literal, NamedTuple, Protocol
...
def get_style(style: Style | None):
    "get the the aut style if exists or None or the past style pram"
    if style:
        return style
...
class Block:
    ...
    def __init__(self, raw: str) -> None:
        self._raw = raw  # unsafe, but might be required later
...
class Bar:
    "Bar Separates Multiple Items In One Line"
...
class Icon:
    "Icon Design, An Icon For Different Names"
...
class Chain:
    "Chain Design, A Chain For A Chain Of Values"
...
def get_logger() -> PrettyLogging:
    "if `global_logger` exists it will just return ir, if not it will create and then remove it"
...
class Pointer:
    "Four Directional Pointer Design"
    left: str = " "
    right: str = " "
    up: str = " "
    down: str = " "
...
class Ruler:
    "A Ruler Object that can display a length of numbers"

    def __init__(self, length: int, style: Style | None = None) -> None:
        self.length = length
        self.style = get_style(style)
    ...
    def __str__(self) -> str:
        ...
        if number_on_top == False:
            result += begin + (line * (self.length * 3)) + end + "\n"
...
class List:
    "A List Display Of multiple items"
...
class Paginator:
    ...
    def __str__(self):
        if self.style:
            ...
            active_page = self.style.get("Paginator.active", "o")
...
def _t(_type: type[BaseException], err: BaseException, trace: TracebackType | None):
    print(
        fx.bold(
            fg.red(
                fx.blink(
                    ascii_border(
                        f"\n{Padding.center(f"Sorry! We Have Encountered An Error.")}\n\n{Padding.left( "(Details): ")}\n{Padding.left(f"      -- name: {str(err)!r}", 5)}\n{Padding.left(f"      -- line: {trace.tb_lineno}", 5)}\n{Padding.left(f"      -- func: {trace.tb_frame.f_code.co_name}", 5)}\n"
                    )
                )
            )
        )
